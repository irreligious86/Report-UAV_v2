/**
 * Фрагмент бібліотеки mgrs v2.1.0 (npm / unpkg), той самий алгоритм, що й у додатку
 * UAV Report: import { toPoint } from "https://esm.sh/mgrs@2.1.0" у js/screens/map.js
 * toPoint(mgrs) → [longitude, latitude] (WGS84).
 *
 * У проєкті Apps Script додайте цей файл поруч із Code.gs (Файл → Додати файл).
 */
/**
...
function forward(ll, accuracy) {
  ...
  if (lat < -80 || lat > 84) {
    throw new TypeError(`forward received a latitude of ${lat}, but this library does not support conversions of points in polar regions below 80Â°S and above 84Â°N`);
  }
 ...
 *
 * @private
 * @param {string} e The first letter from a two-letter MGRS 100Â´k zone.
